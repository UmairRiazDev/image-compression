...
public class MainAlgorithm {
	...
	public MainAlgorithm(String imageDir, String fileDir, String encodingDummy) {
		...
		try {
			...
			ArrayList<Float> inLine = return2DInLine(transform.finalArray);
			...
			FileWriter f0 = new FileWriter(new File(fileDir));
			//f0.write(inLine.toString());
	...
	public MainAlgorithm(String encodedFileDir, String fileImageOutput) {
		...
		try {
			...
			fr = new Scanner(new File(encodedFileDir));
		} catch (Exception ex) {
			ex.printStackTrace();
		}
		...
		float[][] loool = returnInLine2D(huffDecoded);
	...
	private void saveImage(float[][] finalArray, String dir) {
	...
	private ArrayList<Float> return2DInLine(float[][] rawData) {
		ArrayList<Float> ordenado = new ArrayList<Float>();
