...
public class HaarWavelet {
	//Important pointers to the entire structure
	public static int nInteractions;
	public static float[][] finalArray;
	
	//Pointers for internal use only
	private static float[][] haarwavelado;
	
	public HaarWavelet(float[][] compressedData, int interactions) {
		...
		finalArray = revertHaarWaveletTransforms(compressedData);
	}
	
	public HaarWavelet(float[][] rawData) {
		//Transformation based on optimization
		finalArray = optimizeHaarWaveletTransforms(rawData);
	}
	...
	public static void main(String[] args) {
		...
		*/
		//Make Transformation method complete
		/*
	...
	private static float[][] returnItFlat(float rawData[][], int lengthOriginal) {
		...
		if (lengthOriginal < countGCDNumber(rawData.length, 0)) {
			...
			if (lengthOriginal == 0) {
				...
				/*
				System.out.println("First Transformation Level:");
...
private static float[][] revertHaarWaveletTransforms(float rawData[][]) {			
			if (nInteractions == 1) {
				...
				//We will need to climb the mountain on the basis of interactions, only to get the first blocks all together, no.
				...
				/*
				//Try to print all in
				System.out.println("Printing already in reverse in "+haarwavelado.length);
				for (int q=0; q<z.size(); q++)
					System.out.println(Arrays.deepToString(z.get(q)));
				System.out.println();
				
				//Take the last one, make it inverse. Join the previous one, inverse. until 0, inverse. Give back.
